...
def get_image_url(fighter_name):
    ...
    try:
        ...
        # Find the first link to an athlete page
        athlete_link = soup.find('a', href=lambda href: href and '/athlete/' in href)
        
        if athlete_link:
            athlete_url = base_url + athlete_link['href']
            print(f"Athlete URL: {athlete_url}")
            
            resp = requests.get(athlete_url)
            soup = BeautifulSoup(resp.text, "html.parser")
...
for category in rankings_data:
    if category['category'] == '페더급':
        ...
        break # only process one for debugging
